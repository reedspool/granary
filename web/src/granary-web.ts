...
export const initialize = () => {
  ...
  host.onStepped(
    (ctx) => {
      ...
      const resetCause = parseRule("|:@host step: reset all stacks|")
        .causes[0]!;
      if (matchesCause(ctx, additionCause)) {
        ...
        if (!popped)
          throw new Error(
            `Unexpected empty or non-existant stack '${stackName}'`,
          );
      ...
      if (matchesCause(ctx, resetCause)) {
        maybePopMatchingCause(ctx, resetCause);
        setTimeout(() => {
          ctx.stacks = {};
  ...
  return { host, abort: () => controller.abort(), signal };
};
